...
function LoginPage() {
    ...
    try {
      const response = await fetch('https://31.34.150.223/api/auth/local', {
        method: 'POST',
        headers: {
    ...
    <div>
          ...
          <form onSubmit={handleSubmit}>
              <div>
                  ...
                  <input class="connexion-input" type="email" id="email" value={email} onChange={(e) => setEmail(e.target.value)} required />
              </div>
              <div>
                  <label htmlFor="password" class="connexion-label">Mot de passe</label>
                  <input class="connexion-input" type="password" id="password" value={password} onChange={(e) => setPassword(e.target.value)} required />
              </div>
              <button type="submit" class="connexion-bouton">Se connecter</button>
          </form>
        {error && <p>{error}</p>}

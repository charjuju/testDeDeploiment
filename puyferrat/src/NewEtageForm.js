...
function NewEtageForm(id) {
  ...
  async function getRestaurantData(token, id) {
    console.log(id.id);
    const response = await fetch(`https://31.34.150.223/api/restaurants/${id.id}?populate=etages`, {
      headers: {
  ...
  function updateRestaurantData(restoId, data, token) {
    ...
    fetch(`https://31.34.150.223/api/restaurants/${restoId}`, requestOptions)
      .then(response => {
        if (!response.ok) {
  ...
  const handleSubmit = (event) => {
    ...
    fetch("https://31.34.150.223/api/etages/", {
      method: "POST",
      headers: {
